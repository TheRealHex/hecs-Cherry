...
/* 
 * Horizontal position on keyboard for each key as they are located on my Keyboard
 */
...
int main(int argc, char **argv)
{
	...
	const char *env_path = getenv("HECS_OGG_DIR");
	if (env_path) {
		opt_path_audio = env_path;
	}

	printd("Using ogg dir: \"%s\"\n", opt_path_audio);
...
static void usage(char *exe)
{
	fprintf(stderr, 
		...
		"  -p, --audi-path=PATH      load .ogg files from directory PATH\n"
...
/*
 * Play audio file for given keycode. Ogg files are loaded on demand
 */
	...
	if(src[idx] == 0) {
		...
		if(buf[idx] == 0) {
			...
			if(opt_fallback_sound) {
				snprintf(fname, sizeof(fname), "%s/%02x-%d.ogg", opt_path_audio, 0x31, press);
				buf[idx] = alureCreateBufferFromFile(fname);
			} else {
